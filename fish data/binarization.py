# -*- coding: utf-8 -*-
"""
Created on Thu Feb 25 01:27:37 2021
...
def binarize(vol,vol_seg,verbose):
    ...
    vol_base_3 = np.zeros(vol.shape,dtype=np.uint8) # seg+otsu
    ...
    for i in range(slc):
        ...
        # output 2
        diffuse = anisotropic_diffusion(im,niter=3,option=2).astype(np.float32)
        im_enhance = ContrastEnhance(diffuse)
        ...
        otsu_th_3 = threshold_otsu(im_seg)
        vol_base_3[:,i,:] = np.uint8(im_seg > otsu_th_3)*255
        ...
        if verbose == True and i == idx:
            top = np.concatenate((im,vol_base_1[:,i,:],vol_base_2[:,i,:]),axis=1)
            bot = np.concatenate((im_seg,vol_base_3[:,i,:],vol_opt[:,i,:]),axis=1)
...
#            plt.savefig("E:\\OCTA\\result\\vis.jpg")
    ...
    return vol_base_1, vol_base_2, vol_base_3, vol_opt 
...
for i in range(1,7):
    ...
    vol_base_1, vol_base_2, _, vol_opt = binarize(vol_x,vol_latent,True)
